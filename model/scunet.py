...
class SCUNet(nn.Module):
    ...
    def forward(self, x0):
        ...
        if paddingBottom:
            pad_bottom = x0[:, :, -paddingBottom:, :]
            x0 = torch.cat((x0, pad_bottom), dim=2)
        if paddingRight:
            pad_right = x0[:, :, :, -paddingRight:]
            x0 = torch.cat((x0, pad_right), dim=3)

        
        # x0 = nn.ReplicationPad2d((0, paddingRight, 0, paddingBottom))(x0)
        # cv2.imwrite('x0.png', (x0[0]*255).permute(1,2,0).cpu().numpy())
        x1 = self.m_head(x0)
        x2 = self.m_down1(x1)
        x3 = self.m_down2(x2)
        x4 = self.m_down3(x3)
